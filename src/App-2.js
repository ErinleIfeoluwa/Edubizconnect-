    ...
    <div style={{ display:"flex",flexDirection:"column",flex:1 }}>
      ...
      <div style={{ padding:"0 24px",flex:1,overflowY:"auto" }}>
        ...
        <div style={{ paddingBottom:32 }}>
          ...
          {/* Telegram — locked until payment */}
          <div style={{ marginBottom:12 }}>
            <div style={{ display:"flex",alignItems:"center",justifyContent:"center",gap:10,width:"100%",padding:15,borderRadius:20,background:"rgba(34,158,217,0.15)",border:"2px dashed #229ED9",color:"#229ED9",fontSize:15,fontWeight:700,textAlign:"center",boxSizing:"border-box",position:"relative",opacity:0.7 }}>
              <span style={{ fontSize:20 }}>🔒</span> Join Telegram Channel
              <span style={{ position:"absolute",right:14,fontSize:12,background:"#229ED9",color:"white",padding:"3px 8px",borderRadius:50,fontWeight:700 }}>After Payment</span>
            </div>
            <div style={{ fontSize:12,color:T.soft,textAlign:"center",marginTop:6 }}>Complete payment → get Telegram access instantly via WhatsApp</div>
          </div>

          {/* WhatsApp group — locked until payment */}
          <div style={{ marginBottom:12 }}>
            <div style={{ display:"flex",alignItems:"center",justifyContent:"center",gap:10,width:"100%",padding:15,borderRadius:20,background:"rgba(37,211,102,0.1)",border:"2px dashed #25D366",color:"#25D366",fontSize:15,fontWeight:700,textAlign:"center",boxSizing:"border-box",opacity:0.7,position:"relative" }}>
              <span style={{ fontSize:20 }}>🔒</span> Join WhatsApp Group
              <span style={{ position:"absolute",right:14,fontSize:12,background:"#25D366",color:"white",padding:"3px 8px",borderRadius:50,fontWeight:700 }}>After Payment</span>
            </div>
          </div>

          {/* WhatsApp direct chat — always available */}
...
// REFERRAL SYSTEM
// ═══════════════════════════════════════════════
function ReferralScreen({ T, userName, showToast, setScreen }) {
  const referralCode = (userName || "USER").toUpperCase().replace(/\s/g,"").slice(0,6) + "EDU";
  const referralLink = `https://edubizconnect.vercel.app?ref=${referralCode}`;
  const [stats] = useState({ signups: 3, payments: 1, earned: 1 });
  const [history] = useState([
    { name:"Chukwuemeka A.", type:"signup", reward:"Free Course Unlocked", time:"2 days ago", status:"pending" },
    { name:"Fatima M.", type:"payment", reward:"Free Course Unlocked", time:"4 days ago", status:"completed" },
    { name:"David O.", type:"signup", reward:"Free Course Unlocked", time:"1 week ago", status:"pending" },
  ]);
  ...
  function shareLink() {
    if (navigator.share) {
      navigator.share({
        title: "Join EduBiz Connect!",
        text: `🎓 I'm learning on EduBiz Connect — join me and get started on your tech career! Use my referral link:`,
        url: referralLink,
      });
    } else {
      copyLink();
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", flex:1 }}>
      <div style={{ background:T.headerBg, padding:"52px 24px 28px", position:"relative", overflow:"hidden", transition:"background 0.3s" }}>
        <div style={{ position:"absolute", top:-60, right:-60, width:200, height:200, borderRadius:"50%", background:"radial-gradient(circle,rgba(255,92,0,0.3),transparent 70%)", pointerEvents:"none" }} />
        <button onClick={()=>setScreen("profile")} style={{ background:"rgba(255,255,255,0.1)", border:"none", color:"white", fontSize:14, cursor:"pointer", padding:"6px 12px", borderRadius:8, marginBottom:12, display:"block" }}>← Back</button>
        <div style={{ fontSize:20, fontWeight:900, color:"white", position:"relative", zIndex:1 }}>🎁 Referral Program</div>
        <div style={{ fontSize:13, color:"rgba(255,255,255,0.5)", marginTop:4, position:"relative", zIndex:1 }}>Invite friends · Earn free courses</div>
      </div>

      <div style={{ flex:1, padding:"20px 24px", overflowY:"auto" }}>

        {/* How it works */}
        <div style={{ padding:20, background:T.card, borderRadius:20, border:`1px solid ${T.border}`, marginBottom:20, boxShadow:T.shadow }}>
          <div style={{ fontSize:15, fontWeight:800, color:T.dark, marginBottom:14 }}>⚡ How It Works</div>
          {[
            ["1️⃣", "Share your unique referral link"],
            ["2️⃣", "Friend signs up → you earn a free course"],
            ["3️⃣", "Friend pays for a course → you earn 5% bonus"],
            ["4️⃣", "Redeem rewards for any course on EduBiz!"],
          ].map(([ic, text]) => (
            <div key={text} style={{ display:"flex", gap:12, alignItems:"center", marginBottom:12 }}>
              <span style={{ fontSize:20 }}>{ic}</span>
              <span style={{ fontSize:14, color:T.dark, lineHeight:1.4 }}>{text}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:20 }}>
          {[["👥", stats.signups, "Signups"], ["💳", stats.payments, "Payments"], ["🎓", stats.earned, "Free Courses"]].map(([ic, v, l]) => (
            <div key={l} style={{ padding:"14px 8px", background:T.card, borderRadius:16, border:`1px solid ${T.border}`, textAlign:"center", boxShadow:T.shadow }}>
              <div style={{ fontSize:22 }}>{ic}</div>
              <div style={{ fontSize:22, fontWeight:900, color:"#FF5C00", marginTop:4 }}>{v}</div>
              <div style={{ fontSize:11, color:T.soft, marginTop:2 }}>{l}</div>
            </div>
          ))}
        </div>

        {/* Referral code */}
        <div style={{ padding:20, background:`linear-gradient(135deg,${T.headerBg},#2E1A00)`, borderRadius:20, marginBottom:20, position:"relative", overflow:"hidden" }}>
          <div style={{ position:"absolute", top:-30, right:-30, width:120, height:120, borderRadius:"50%", background:"radial-gradient(circle,rgba(255,92,0,0.3),transparent 70%)" }} />
          <div style={{ fontSize:13, color:"rgba(255,255,255,0.5)", marginBottom:8 }}>Your Referral Code</div>
          <div style={{ fontSize:32, fontWeight:900, color:"#FF5C00", letterSpacing:4, marginBottom:16 }}>{referralCode}</div>
          <div style={{ padding:"12px 16px", background:"rgba(255,255,255,0.08)", borderRadius:12, marginBottom:16, wordBreak:"break-all" }}>
            <div style={{ fontSize:12, color:"rgba(255,255,255,0.4)", marginBottom:4 }}>Your referral link</div>
            <div style={{ fontSize:12, color:"white" }}>{referralLink}</div>
          </div>
          <div style={{ display:"flex", gap:10 }}>
            <button onClick={copyLink} style={{ flex:1, padding:"13px", borderRadius:14, background:"rgba(255,255,255,0.1)", border:"1px solid rgba(255,255,255,0.2)", color:"white", fontWeight:700, fontSize:14, cursor:"pointer" }}>📋 Copy Link</button>
            <button onClick={shareLink} style={{ flex:1, padding:"13px", borderRadius:14, background:"linear-gradient(135deg,#FF5C00,#FF8A3D)", border:"none", color:"white", fontWeight:700, fontSize:14, cursor:"pointer" }}>🚀 Share Now</button>
          </div>
        </div>

        {/* Share via */}
        <div style={{ padding:20, background:T.card, borderRadius:20, border:`1px solid ${T.border}`, marginBottom:20, boxShadow:T.shadow }}>
          <div style={{ fontSize:15, fontWeight:800, color:T.dark, marginBottom:14 }}>📤 Share Via</div>
          <div style={{ display:"flex", gap:10 }}>
            <a href={`https://wa.me/?text=🎓 Join EduBiz Connect and start your tech career! Use my referral link: ${referralLink}`} target="_blank" rel="noopener noreferrer" style={{ flex:1, padding:"14px 10px", borderRadius:14, background:"#25D366", color:"white", fontWeight:700, fontSize:13, textAlign:"center", textDecoration:"none" }}>💬 WhatsApp</a>
            <a href={`https://t.me/share/url?url=${referralLink}&text=🎓 Join EduBiz Connect!`} target="_blank" rel="noopener noreferrer" style={{ flex:1, padding:"14px 10px", borderRadius:14, background:"#229ED9", color:"white", fontWeight:700, fontSize:13, textAlign:"center", textDecoration:"none" }}>✈️ Telegram</a>
            <a href={`https://twitter.com/intent/tweet?text=🎓 Learning tech on EduBiz Connect! Join me:&url=${referralLink}`} target="_blank" rel="noopener noreferrer" style={{ flex:1, padding:"14px 10px", borderRadius:14, background:"#1DA1F2", color:"white", fontWeight:700, fontSize:13, textAlign:"center", textDecoration:"none" }}>🐦 Twitter</a>
          </div>
        </div>

        {/* Referral history */}
        <div style={{ fontSize:16, fontWeight:800, color:T.dark, marginBottom:14 }}>📋 Referral History</div>
        {history.map((h, i) => (
          <div key={i} style={{ display:"flex", gap:14, padding:"14px 16px", background:T.card, borderRadius:16, marginBottom:10, border:`1px solid ${T.border}`, boxShadow:T.shadow }}>
            <div style={{ width:44, height:44, borderRadius:14, background:T.orangePale, display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, flexShrink:0 }}>{h.type==="payment"?"💳":"👤"}</div>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:14, fontWeight:700, color:T.dark }}>{h.name}</div>
              <div style={{ fontSize:12, color:"#FF5C00", marginTop:2, fontWeight:600 }}>{h.reward}</div>
              <div style={{ fontSize:11, color:T.soft, marginTop:2 }}>{h.time}</div>
            </div>
            <div style={{ padding:"4px 10px", borderRadius:50, background:h.status==="completed"?"rgba(16,185,129,0.1)":"rgba(245,158,11,0.1)", color:h.status==="completed"?"#10B981":"#F59E0B", fontSize:11, fontWeight:700, height:"fit-content", alignSelf:"center" }}>
        ...
        {/* Redeem */}
        <div style={{ padding:20, background:T.orangePale, borderRadius:20, border:`2px solid ${T.border}`, marginTop:8, marginBottom:20 }}>
          <div style={{ fontSize:15, fontWeight:800, color:"#FF5C00", marginBottom:8 }}>🎓 Redeem Your Free Course</div>
          <div style={{ fontSize:13, color:T.soft, lineHeight:1.6, marginBottom:14 }}>You have <strong style={{color:"#FF5C00"}}>1 free course</strong> to redeem! Contact us on WhatsApp or Telegram to claim it.</div>
          <div style={{ display:"flex", gap:10 }}>
            <a href="https://wa.me/2349022953830?text=Hi! I want to redeem my free course. My referral code is: ${referralCode}" target="_blank" rel="noopener noreferrer" style={{ flex:1, padding:"12px", borderRadius:12, background:"#25D366", color:"white", fontWeight:700, fontSize:13, textAlign:"center", textDecoration:"none" }}>💬 Redeem on WhatsApp</a>
            <a href="https://t.me/+BV0mkBqNuto1NGE0" target="_blank" rel="noopener noreferrer" style={{ flex:1, padding:"12px", borderRadius:12, background:"#229ED9", color:"white", fontWeight:700, fontSize:13, textAlign:"center", textDecoration:"none" }}>✈️ Redeem on Telegram</a>
          </div>
        </div>
  ...
  return (
    <div style={{ fontFamily:font,maxWidth:430,margin:"0 auto",minHeight:"100vh",background:T.bg,display:"flex",flexDirection:"column",position:"relative",overflowX:"hidden",transition:"background 0.3s" }}>
      <div style={{ flex:1,display:"flex",flexDirection:"column",overflowY:"auto" }}>
        ...
        {screen==="referral" && <ReferralScreen T={T} setScreen={setScreen} showToast={showToast} userName={userName} />}
